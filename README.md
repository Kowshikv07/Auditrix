...
- The **`streaming_long_horizon` task** (350 records, 400 steps, sparse terminal-focused reward with low-magnitude shaping) directly tests the sub-theme requirement: _"agents must decompose goals, track state over extended trajectories, and push beyond shallow next-token reasoning."_
- The **`prioritize_rules` action** forces the agent to commit to a multi-step strategy _before_ seeing any rewards — a true long-horizon planning signal.
...
### 🌊 Streaming — Long-Horizon Multi-Rule Priority Audit _(new for Theme #2)_
...
**Mercor Bonus alignment:** The `generate_report` terminal reward scores the report's _quality_, which scales with report completeness — the `flagged_violations` list, `compliant_records` list, `recommendations` field, and crucially the `audit_confidence` section (evidence coverage ratio, high-confidence flags, uncertain flags). A richer, longer report yields a meaningfully higher terminal reward, creating a token-output reward gradient.
...
## Benchmark Visual Results

The following plots summarize baseline vs RL-trained performance across all tasks.

### 1) Task-Level Metric Snapshot

Detection, precision, coverage, efficiency, and final score for each task:

![Task-level metrics table](assets/Task.png)

### 2) Baseline vs RL-Trained Score Table

Direct score comparison and relative improvement per task:

![Baseline vs RL-trained score table](assets/table.png)

### 3) Multi-Metric Radar View

High-level task profile across detection, precision, and coverage:

![Performance radar chart](assets/web.png)

### 4) Score Comparison Across Tasks

Side-by-side bars for baseline (72B) and RL-trained (3B):

![Score comparison bar chart](assets/score_comp.png)

### 5) RL Improvement Over Baseline

Relative gain (%) by task:

![RL improvement chart](assets/rl_improv.png)

### 6) Final Score Formula Weights

Weight allocation used in the final scoring formula:

![Scoring component weight chart](assets/Pie.png)
...
1. **Rule-based logic with exemptions** — inactive employees are exempt from R5/R8; records without PII access are exempt from R9. Agents must understand _when not to apply_ a rule.
...
5. **Precision penalties** — false positives cost -0.30 per flag, forcing _evidence-based flagging_ over heuristic pattern matching.
